...
bool PtyProcess::DoStart(const char *cmd, const Vector<String> *args, const char *env, const char *cd)
{
	...
	if(attrs && tcsetattr(slave, TCSANOW, attrs.Get()) < 0)
		LLOG("tcsetattr() failed.");
...
{
	bool b = master >= 0 && IsRunning() && !IsNull(sz);
	if(b) {
		struct winsize wsz;
		Zero(wsz);
		wsz.ws_col = max(2, sz.cx);
		wsz.ws_row = max(2, sz.cy);
		b = ioctl(master, TIOCSWINSZ, &wsz) >= 0;
		if(b) {
			ptysize = sz;
			LLOG("Pty size is set to: " << ptysize);
		}
	}
	else { ptysize = sz; return true; }
	return b;
}
...
bool PtyProcess::SetAtrributes(const termios& t)
{
	attrs = MakeOne<termios>(t);
	bool b = master >= 0 && IsRunning();
	if(b) {
		 b = tcsetattr(master, TCSAFLUSH, &t) >= 0;
		 if(b) LLOG("Pty attributes changed.");
	}
	else { return true; }
	return b;
}
